"""白名单子进程：仅执行 F04B 专项及注册状态下的 F04A 回归。"""
...
def child(mode):
    ...
    require(
        os.environ.get("DJANGO_SETTINGS_MODULE") == "experiments.wagtail_f04b.settings", "配置错误"
    )
    import django
    ...
    result = Runner(verbosity=2, interactive=False).run_tests(
        ["experiments.wagtail_f04b.tests", "experiments.wagtail_f04a.tests"]
    )
...
def main():
    ...
    if len(sys.argv) == 3 and sys.argv[1] == "--child":
        require(sys.argv[2] == "test", "本轮只允许测试")
        return child(sys.argv[2])
    require(not sys.argv[1:], "本轮只允许测试")
    mode = "test"
    env = {k: v for k, v in os.environ.items() if k.upper() in SYSTEM_ENV}
    env.update(DJANGO_SETTINGS_MODULE="experiments.wagtail_f04b.settings", PYTHONUTF8="1")
