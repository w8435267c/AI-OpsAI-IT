...
def child(mode):
    ...
    require(
        os.environ.get("DJANGO_SETTINGS_MODULE") == "experiments.wagtail_f04a.settings", "配置错误"
    )
    import django
    ...
    if mode == "makemigrations":
        call_command("makemigrations", "fusion_f04a", interactive=False)
        result = 0
    else:
        ...
        result = Runner(verbosity=2, interactive=False).run_tests(
            ["experiments.wagtail_f04a.tests"]
        )
...
def main():
    ...
    env.update(DJANGO_SETTINGS_MODULE="experiments.wagtail_f04a.settings", PYTHONUTF8="1")
