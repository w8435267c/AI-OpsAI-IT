...
def main():
    ...
    if Path(sys.prefix).resolve() != root / ".venv":
        raise RuntimeError("必须使用融合工作目录的现有 .venv")
    sys.path.insert(0, str(root))
    legacy = root / "experiments" / "wagtail_f07"
    sys.path.insert(0, str(legacy))
    suite = unittest.TestSuite(
        [
            unittest.defaultTestLoader.loadTestsFromName("experiments.wagtail_f08.test_body_text"),
            unittest.defaultTestLoader.discover(str(legacy), pattern="test_conversion.py"),
        ]
    )
