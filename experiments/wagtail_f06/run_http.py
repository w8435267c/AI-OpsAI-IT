"""F06 HTTP 与详情回归；合成配置及白名单/审计防护，仅内存测试。"""
...
def child():
    ...
    require(
        os.environ.get("DJANGO_SETTINGS_MODULE") == "experiments.wagtail_f06.settings",
        "配置错误",
    )
    import django
    ...
    result = Runner(verbosity=2, interactive=False).run_tests(
        ["experiments.wagtail_f06.test_http", "experiments.wagtail_f06.test_readers"]
    )
...
def main():
    ...
    env.update(DJANGO_SETTINGS_MODULE="experiments.wagtail_f06.settings", PYTHONUTF8="1")
