"""F07 顺序幂等、异常回滚及转换回归；合成配置及白名单/审计防护，仅内存测试。"""
...
def child(make=False):
    ...
    require(
        os.environ.get("DJANGO_SETTINGS_MODULE") == "experiments.wagtail_f07.settings",
        "配置错误",
    )
    import django
    ...
    if make:
        call_command("makemigrations", "fusion_f07", interactive=False)
    ...
    result = Runner(verbosity=2, interactive=False).run_tests(
        [
            "experiments.wagtail_f07.test_import",
            "experiments.wagtail_f07.test_integration",
            "test_conversion",
        ]
    )
    require(not violations, "存在隔离违规")
...
def main():
    ...
    if sys.argv[1:] in (["--child"], ["--child", "make"]):
        return child(sys.argv[-1] == "make")
    require(sys.argv[1:] in ([], ["make"]), "仅接受 make 或默认测试")
    env = {k: v for k, v in os.environ.items() if k.upper() in SYSTEM_ENV}
    env.update(DJANGO_SETTINGS_MODULE="experiments.wagtail_f07.settings", PYTHONUTF8="1")
