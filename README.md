...
- 当前阶段：**核心模型、迁移文件、系统操作角色与本地模拟登录已建立，内容受众 Selector 及第 8F 联合保存已完成，第 8G 定点复核通过**。
...
- accounts 用户、部门、多部门关系及内容用户组模型；
- knowledge 核心模型及内容受众 Selector（尚未接入页面、HTTP API、搜索或附件）；
- 系统操作角色、本地模拟登录及后台权限保护；
- `/health/live` 存活检查及自动化测试；
- Docker Desktop（WSL 2 后端）与 PostgreSQL 本地开发环境（`deploy/compose.yaml`，db + web）；
- `/health/ready` 就绪检查及自动化测试。

迁移文件已存在：`apps/accounts/migrations` 的 0001～0002、`apps/knowledge/migrations` 的 0001～0003。knowledge.0003 仅同步空间默认受众策略的字段注释。此前报告记录两模块的 0001、0002 已在 PostgreSQL 应用，knowledge.0003 当时未应用；迁移文件存在不代表当前持久数据库已更新，实际应用状态及数据库注释仍待核验。

第 8F 支持常规联合编辑、非循环更新链及授权删除／修改／新增混合操作。循环交换首版不支持，在表单阶段返回中文提示、保留输入、整次不保存；不删除重插既有规则，不改变数据库约束或角色权限。Article 编号服务及发布事务尚未实现，Article Admin 新增入口仍关闭。

以下验证成绩引用用户提供的此前完成报告，不是第 8H 文档同步时重新执行的结果：

- 第 8G：按调整后的范围通过；项目专项 76 passed、仓库外补充复现 12 passed；Django check、迁移一致性、Ruff 静态与格式、工作区及提交差异空白检查通过。第 8G 未重跑完整套件。
- 第 8F 历史验证：完整套件 386 passed、11 subtests passed。

第 8F 确定性保存缺陷已按当前契约关闭；PostgreSQL 及持久库迁移仍需另行安排。下一步安排 PostgreSQL 验证准备，具体数据库操作另行限定范围：

- PostgreSQL 查询、排序及相关数据库行为待验证；持久数据库迁移状态、实际数据库注释待核验。
- 未新增并发协调锁，实际并发及锁行为未验证；并发写入仍可能冲突或覆盖更新，事务回滚不等同于并发安全。
- 不自动进入数据库操作、双端推送或第 9 步；同步状态以对 GitHub、Gitee 的实时核验为准。
