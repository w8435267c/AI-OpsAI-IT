...
依据：PRD V1.1A 第 8 章“数据模型与字段要求”和第 9 章“业务规则与状态模型”。

外部依赖：
1. settings.AUTH_USER_MODEL 指向 accounts.User。
2. accounts.Department 表示同步自钉钉的部门。
3. accounts.UserGroup 表示内容受众用户组，不等同于系统操作角色。

说明：
- 为使 Article 的外键可直接迁移，本文件同时给出最小必要的
  KnowledgeSpace 和 Category 支撑模型。
- 文章编号 kb_no 应由并发安全的服务或数据库序列生成；模型只负责格式、
  唯一性和不可编辑属性，不使用“查询最大编号再加一”的不安全写法。
"""
    ...
    def clean(self) -> None:
        ...
        if self.latest_working_version_id:
            ...
            if version.article_id != self.id:
                ...
                errors["latest_working_version"] = _(
                    "最新工作版本只能是草稿、待审核或已驳回状态。"
                )
...
class ArticleVersion(models.Model):
    ...
    class Meta:
        ...
        constraints = [
            ...
            models.CheckConstraint(
                condition=(
                    ~Q(status=VersionStatus.PUBLISHED)
...
class ArticleAudience(models.Model):
    ...
    def __str__(self) -> str:
        return f"{self.article.kb_no} {self.get_audience_type_display()} {self.get_effect_display()}"
